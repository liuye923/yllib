...
#            v = v/v.std(dim='time')
    ...
    def calc_cluster_mapsize_fast(self, data, bmu, mapsize=None, rdim='time'):
#        data = data.assign_coords(bmu=("time", bmu.data))#["bmu"] = bmu.astype(np.float)
        ...
        for ibmu in np.arange(mapsize[0] * mapsize[1]):
            ...
            if cluster.coords[rdim].size==0:
                ...
                cluster_mean.append(cluster.mean(rdim))
                pct.append(len(cluster.coords[rdim]) / len(data.coords[rdim]) * 100.)
        return cluster_mean, pct

    def calc_cluster_mapsize(self, data, bmu, mapsize=None, rdim='time'):
        data = data.assign_coords(bmu=(rdim, bmu.data))#["bmu"] = bmu.astype(np.float)
        clusters = []
        cluster_mean = []
        pct = []
        for ibmu in np.arange(mapsize[0] * mapsize[1]):
